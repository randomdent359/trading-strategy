#!/usr/bin/env python3
"""
Polymarket Contrarian Monitor
Watches for consensus extremes (>72% on one outcome) and alerts

Theory: When consensus is extremely extreme, contrarians usually win
Win rate target: 54%
"""
...
class ContrariandMonitor:
    def __init__(self):
        ...
        self.log_file = self.logs_dir / "contrarian-monitor.log"
        self.alerts_file = self.data_dir / "consensus-extremes.jsonl"
        self.state_file = self.data_dir / "monitor-state.json"
        
        self.interval = 30
        self.min_consensus = 0.72  # >72% consensus = contrarian opportunity
    ...
    def run(self):
        self.log("=" * 80)
        self.log("🎲 POLYMARKET CONTRARIAN MONITOR")
        self.log(f"Polling: {self.interval}s | Min consensus: {self.min_consensus*100:.0f}%")
            ...
            try:
                ...
                if markets:
                    ...
                    if extremes:
                        ...
                        self.log(f"🚨 Poll #{self.poll_count}: {len(markets)} markets | {len(extremes)} consensus extremes (cumulative: {self.extreme_count})")
                    else:
                        if self.poll_count % 10 == 0:
                            self.log(f"✓ Poll #{self.poll_count}: {len(markets)} markets, no >72% extremes")
                else:
