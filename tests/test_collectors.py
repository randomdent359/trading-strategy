...
from trading_core.collectors.polymarket import _extract_markets_from_series
...
class TestPolymarketExtraction:
    def test_extract_btc_market(self):
        ...
        rows = _extract_markets_from_series(series_data, ["BTC", "ETH", "SOL"])
        assert len(rows) == 1
    ...
    def test_multiple_series_and_markets(self):
        series_data = [
            {
                "markets": [
                    {
    ...
    def test_respects_asset_filter(self):
        ...
        # Only request BTC
        rows = _extract_markets_from_series(series_data, ["BTC"])
        assert len(rows) == 1
        assert rows[0]["asset"] == "BTC"
